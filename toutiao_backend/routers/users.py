...
from schemas.users import UserRegisterRequest, UserAuthResponse, UserInfoResponse, UserUpdateRequest, \
...
# 查 Token 查用户 -> 封装crud -> 功能整合成一个工具函数 -> 路由导入使用：采用依赖注入方式
@router.get("/info")
...
# 修改用户信息：验证 Token  -> 更新（用户输入数据，put提交 -> 请求体参数 -> pydantic 模型） -> 响应结果
# 参数：用户输入 + 验证 Token + db（调用数据库更新的方法）
@router.put("/update")
...
# 修改密码：验证 Token -> 验证旧密码 -> 更新新密码 -> 响应结果
@router.put("/password")
async def change_password(
    ...
    if not res_change_pwd:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="修改密码失败，请稍后在试")
